...
def _save_batch(X, y, batch_id, s3):
    ...
    s3.load_file(filename=X_path, key=f"{PROCESSED_PREFIX}X_{batch_id:04d}.npy",
                 bucket_name=BUCKET_NAME, replace=True)
    s3.load_file(filename=y_path, key=f"{PROCESSED_PREFIX}y_{batch_id:04d}.npy",
                 bucket_name=BUCKET_NAME, replace=True)
...
def _preprocess_mri_images():
    ...
    s3.load_string(string_data=json.dumps(class_to_idx),
